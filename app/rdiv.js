...
function rdiv_calc(data, calc_type) {
    ...
    if (calc_type === "rdiv_vin")
        data.vin = _vin;
    if (calc_type === "rdiv_vout")
        data.vout = _vout;
    if (calc_type === "rdiv_rl")
        data.rl = _rl;
    if (calc_type === "rdiv_rh")
        data.rh = _rh;
}
// ── Session ───────────────────────────────────────────────────
const RDIV_ROOT_SESSION = {
    ...
    _on_click(e) {
        ...
        if (cell)
            cell.textContent = data.ok ? data[keys[id]].toFixed(2) + " " + units[id] : "NaN";
    },
    on_register(_state) {
        _state.rdiv ?? (_state.rdiv = { last_calc: null });
    },
    on_push(_state) {
