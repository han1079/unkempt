const LOAD_SCREEN_SESSION = {
    ...
    on_register(_state: GlobalState) {
        ...
        const el = document.querySelector<HTMLElement>(".overlay_container");
    ...
    on_event(_event: DispatchedEvent, _state: GlobalState, _requests: SessionRequest[]) {},

    on_dt(dt: number, _state: GlobalState, requests: SessionRequest[]) {
        ...
        const el = document.querySelector<HTMLElement>(".overlay_container");
        if (el) el.style.opacity = String(_state.load_screen_opacity);
        if ((_state.load_screen_opacity as number) < 0.01) {
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    const container = document.querySelector<HTMLElement>("[data-rootsession]");
